...
export const TabelaVendedores = ({
  ...
  return (
    <Cartao
      ...
      nota='Recebidos: chegaram no vendedor no período (primeira entrada em negociação; quem voltou do Break não conta de novo) mais as vendas contadas cujo lead nunca passou por negociação, no dia da venda. A mesma conta da visão Cohort. Ganhos: vendas do período (data de fechamento) com o campo Fechamento = Comercial, para o dono do card; Direto e Recompra ficam fora; sem o campo, conta só se passou por negociação ou se um vendedor marcou o Ganho à mão. Perdidos: perderam no período (data de fechamento), estão em Perdido hoje e passaram por negociação em alguma data; lead descartado na qualificação não entra, e lead recuperado sai da coluna na hora. Em aberto: recebidos ainda sem desfecho. Taxa: ganhos sobre recebidos, ou seja, vendas do período sobre leads que chegaram no período. Receita e ticket: dos ganhos; o ticket é receita dividida pela quantidade de vendas, então venda sem valor preenchido puxa o ticket para baixo.'
      tema={tema}
    >
      {truncado ? (
