...
export const GUIA_VENDEDORES = [
  ...
  'Recebidos: leads que chegaram na pessoa no período, contados do mesmo jeito da visão Cohort: cada negócio uma vez, na primeira vez que entrou em negociação; quem voltou do Break não conta de novo. Venda contada cujo card pulou a etapa de negociação também entra, no dia da venda. Em aberto: desses, os que ainda não são Ganho nem Perdido. Perdidos: estão em Perdido hoje, com a data de fechamento dentro do período, e passaram por negociação em alguma data; lead descartado ainda na qualificação não entra, porque nenhum vendedor chegou a trabalhar nele. Por isso Recebidos − Ganhos − Perdidos não dá Em aberto.',
