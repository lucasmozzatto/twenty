...
export const GUIA_VENDEDORES = [
  ...
  'Recebidos: leads que chegaram na pessoa no período, contados do mesmo jeito da visão Cohort: cada negócio uma vez, na primeira vez que entrou em negociação; quem voltou do Break não conta de novo. Venda contada cujo card pulou a etapa de negociação também entra, no dia da venda. Em aberto: desses, os que ainda não são Ganho nem Perdido. Perdidos: foram criados no período, estão em Perdido hoje e passaram por negociação em alguma data; lead descartado ainda na qualificação não entra, porque nenhum vendedor chegou a trabalhar nele. Se o vendedor recuperar um perdido e vender, ele sai desta coluna e entra em Ganhos. Por isso Recebidos − Ganhos − Perdidos não dá Em aberto.',
  'Ganhos seguem a data de fechamento do card. Perdidos seguem a data de criação do lead: a coluna fala do lote que entrou no período, e não das perdas de leads antigos que a cadência encerrou agora. Recebidos e Em aberto leem o histórico de etapas e contam a partir de 01/09/2026.',
